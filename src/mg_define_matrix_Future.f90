...
!!  ----------------------------------------------------------------------------------------------------------------
    ...
    do i = 1,nx
       ...
       enddo
    enddo
    !!! Bottom level. A bit tricky :)
    k = 1
    do i = 1,nx
       do j = 1,ny
          zxu = zr(k,j,i+1)-zr(k,j,i)/dxu(j,i)
          zyu = 0.5*(zydx(k,j,i)/dx(j,i)+zydx(k,j,i+1)/dx(j,i+1))
          zxv = 0.5*(zxdy(k,j,i)/dy(j,i)+zxdy(k,j+1,i)/dy(j+1,i))
          zyv = zr(k,j+1,i)-zr(k,j,i)/dyv(j,i)
          cA(3,k,j,i) = 0.25*zydx(k+1,j,i)+0.25*zydx(k,j-1,i)                              !! couples with k+1 j-1
